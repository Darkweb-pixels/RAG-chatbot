...
class ContextAwareChatbot:
    ...
    def chat(self, user_query: str) -> str:
        ...
        # Construct full prompt with specific instruction for point-to-point answers
        full_prompt = f"""
        Relevant Document Contexts:
        {context_str}
        ...
        try:
            chat_completion = self.groq_client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "You are a helpful AI assistant that gives concise, point-to-point answers. You answer directly without unnecessary explanations."},
                    {"role": "user", "content": full_prompt}
                ],
                model="llama3-8b-8192"  # You can change the model as needed
            )
